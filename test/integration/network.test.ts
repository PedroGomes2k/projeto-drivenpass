...
describe('Get /network', () => {
  describe('Get the network', () => {
    ...
    it('should respond 404 when body is empty ', async () => {
      ...
      const response = await api.get('/network').set('Authorization', `Bearer ${token.token}`);
...
describe('Get /network/:Id', () => {
  describe('Get the network', () => {
    ...
    it('should respond 401 when ID does not belong to the user ', async () => {
      ...
      const response = await api.get(`/network/${network.id}`).set('Authorization', `Bearer ${token.token}`);
    ...
    it('should respond 404 when body is empty ', async () => {
      ...
      const response = await api.get(`/network/1`).set('Authorization', `Bearer ${token.token}`);
...
describe('Post /network', () => {
  ...
  describe('Token is valid', () => {
    it('should respond status 400 when body inst given ', async () => {
      const token = await generateValidToken();
      const body = { [faker.lorem.word()]: faker.lorem.word() };

      const response = await api.post('/network').set('Authorization', `Bearer ${token.token}`).send(body);
      expect(response.status).toEqual(httpStatus.BAD_REQUEST);
    });
    it('should respond status 400 when body isnt present', async () => {
      const token = await generateValidToken();
      const response = await api.post('/network').set('Authorization', `Bearer ${token.token}`);
      expect(response.status).toEqual(httpStatus.BAD_REQUEST);
    });
...
describe('Delete /network', () => {
  ...
  describe('Token is valid', () => {
    it('should respond status 400 when body is not given ', async () => {
      const token = await generateValidToken();

      const response = await api.delete('/network').set('Authorization', `Bearer ${token.token}`).send('');
      expect(response.status).toEqual(httpStatus.BAD_REQUEST);
    });
    it('should respond status 400 when body is not present', async () => {
      const token = await generateValidToken();
      const response = await api.delete('/network').set('Authorization', `Bearer ${token.token}`).send('');
      expect(response.status).toEqual(httpStatus.BAD_REQUEST);
    });
    ...
    it('should respond 404 when body is empty ', async () => {
      ...
      const response = await api.delete('/network').set('Authorization', `Bearer ${token.token}`).send({ id: 1 });
